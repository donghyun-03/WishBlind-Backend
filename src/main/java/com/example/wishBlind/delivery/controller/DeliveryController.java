...
public class DeliveryController {
    ...
    public ApiResponse<DeliveryResponse> submit(@PathVariable Long giftSessionId,
                                                @Valid @RequestBody DeliveryRequest request) {
        return ApiResponse.success(deliveryService.submit(giftSessionId, request));
    }

    @Operation(summary = "전달 정보 조회")
    @GetMapping("/api/gift-sessions/{giftSessionId}/delivery")
    public ApiResponse<DeliveryResponse> get(@PathVariable Long giftSessionId) {
        return ApiResponse.success(deliveryService.get(giftSessionId));
    }
    ...
    public ApiResponse<Void> complete(@PathVariable Long giftSessionId) {
        deliveryService.complete(giftSessionId);
