...
public class RecommendationController {
    ...
    public ApiResponse<List<RecommendationResponse>> generate(@PathVariable Long giftSessionId) {
        return ApiResponse.success(recommendationService.generate(giftSessionId));
    }
    ...
    public ApiResponse<List<RecommendationResponse>> list(@PathVariable Long giftSessionId) {
        return ApiResponse.success(recommendationService.getList(giftSessionId));
    }

    @Operation(summary = "AI 추천 상세 조회", description = "추천 이유·별점·AI 코멘트·고려할 점을 반환.")
    @GetMapping("/api/recommendations/{recommendationId}")
    public ApiResponse<RecommendationDetailResponse> detail(@PathVariable Long recommendationId) {
        return ApiResponse.success(recommendationService.getDetail(recommendationId));
    }
    ...
    public ApiResponse<Void> finalizeSelection(@PathVariable Long giftSessionId,
                                               @Valid @RequestBody FinalizeRequest request) {
        recommendationService.finalizeSelection(giftSessionId, request.recommendationId());
