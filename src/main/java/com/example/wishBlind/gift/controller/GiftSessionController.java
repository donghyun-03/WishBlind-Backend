...
public class GiftSessionController {
    ...
    @PostMapping
    public ApiResponse<GiftSessionResponse> create(@Valid @RequestBody GiftSessionCreateRequest request) {
        return ApiResponse.success(giftSessionService.create(request));
    }

    @Operation(summary = "선물 세션 단건 조회", description = "선물 건 상세 및 현재 상태를 조회한다.")
    @GetMapping("/{id}")
    public ApiResponse<GiftSessionResponse> get(@PathVariable Long id) {
        return ApiResponse.success(giftSessionService.get(id));
    }

    @Operation(summary = "내 선물 목록(홈 대시보드)", description = "최근 진행한 선물을 상태 배지와 함께 최신순으로 반환한다.")
    @GetMapping
    public ApiResponse<List<GiftSessionListResponse>> list() {
        return ApiResponse.success(giftSessionService.getMyList());
    }

    @Operation(summary = "초대 링크/코드 생성", description = "받는 사람에게 보낼 초대 토큰·코드·URL을 발급하고 상태를 '취향 입력 대기'로 전환한다.")
    @PostMapping("/{id}/invite")
    public ApiResponse<InviteResponse> invite(@PathVariable Long id) {
